...
parser = argparse.ArgumentParser(
    "ChatGLM generation script (int8 path)", add_help=False
)
parser.add_argument(
    "-m",
    "--model-id",
...
if args.ipex_weight_only_quantization:
    ...
    with torch.no_grad(), torch.autocast(
        ...
        if amp_enabled:
            convert_model = ipex.optimize(
                convert_model, dtype=torch.bfloat16, inplace=True, concat_linear=False
            )
...
if args.benchmark:
    ...
    if args.prompt is not None:
        ...
        prompt = prompt_pool["chatglm"]["8192"] * int(int(args.input_tokens) / 8192)
    elif args.input_tokens in prompt_pool["chatglm"]:
