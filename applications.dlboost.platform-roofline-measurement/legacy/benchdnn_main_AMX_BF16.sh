...
# Manual Settings
week="guojian_AMX_BF16_E9I.0093.D22"
GROUP="TMULPVP_RUN2"
    ...
    for wltype in $TYPES; do
        ...
        for (( RUN=1; RUN<=$RUNS; RUN++ )); do
            ...
            for kernel in ${onednn_rn50_kernels}; do
                ...
                cmd="tmc -c \"$basecmd\" -D \"BDNN_${wltype}_${sufix}_${week}\" -i \"BENCHDNN ${wltype} MB${batchsize} ${kernel} ${FREQ}Freq ${MEMORY}MTs run${RUN} ${HT} ${COMMENT}\" $UPLOAD -n -x ${USER} -o ${max_duration}"
                if [ "$EMON" == "1" ] ; then
