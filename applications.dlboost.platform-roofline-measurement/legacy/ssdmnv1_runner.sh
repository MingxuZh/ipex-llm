...
if [ "$wlmode" == "amx" ] ; then
    precision=int8
    model_file=ssdmobilenet_int8_pretrained_model_combinedNMS_s8.pb
    export DNNL_MAX_CPU_ISA=AVX512_CORE_AMX
elif [ "$wlmode" == "fp32" ] ; then
    precision=fp32
    model_file=ssdmobilenet_fp32_pretrained_model_combinedNMS.pb
...
if [ "$datatype" == "real" ] ; then
    data_location="--data-location /workspace/dataset/cocoval/coco_val.record"
else
    data_location=""
fi
