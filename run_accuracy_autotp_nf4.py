...
parser.add_argument(
    "--dtype", default="nf4", type=str, help="float32 or bfloat16 or int8 or int4 or nf4"
)
parser.add_argument(
...
class HuggingFaceModel(BaseLM):
    ...
    def __init__(
        ...
        #################################### quantize WOQ NF4 start
        qconfig = ipex.quantization.get_weight_only_quant_qconfig_mapping(
            weight_dtype=ipex.quantization.WoqWeightDtype.NF4,
            lowp_mode=ipex.quantization.WoqLowpMode.BF16,
            group_size=args.group_size,
        )
        print('[info] quantize model to woq nf4')
        self.model = ipex.llm.optimize(
            self.model.eval(),
            dtype=torch.bfloat16 if args.quant_with_amp else torch.float,
        ...
        with torch.inference_mode(), torch.no_grad(), torch.cpu.amp.autocast(
            ...
            if self._with_jit and self.iter == 0:
                if self._dtype not in ["int8", "int4"]:
                    print('[info] jit trace and freeze')
